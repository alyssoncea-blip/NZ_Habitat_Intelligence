...
def load_affordability_data() -> Dict[str, Any]:
    ...
    hero_kpis = {
        "years_to_buy": {
            ...
            "change": 0.5,
            "status": "Expensive" if nat_avg > 8 else ("Moderate" if nat_avg > 6 else "Affordable"),
            "color_scale": "#e74c3c" if nat_avg > 8 else ("#ffc107" if nat_avg > 6 else "#28a745"),
            "sparkline": aff_ts if aff_ts else None,
            "by_region": years_to_buy if years_to_buy else {r: nat_avg + (i - 8) * 1.5 for i, r in enumerate(NZ_REGIONS)},
        },
        "rent_burden": {
            "value": sum(rent_burden.values()) / max(1, len(rent_burden)) if rent_burden else 30.0,
            "trend": "up",
            "change": 1.2,
            "status": "Critical" if (sum(rent_burden.values()) / max(1, len(rent_burden)) if rent_burden else 30.0) > 35 else ("Warning" if (sum(rent_burden.values()) / max(1, len(rent_burden)) if rent_burden else 30.0) > 30 else "Healthy"),
            "threshold": 30,
            "sparkline": rent_ts if rent_ts else None,
            "by_region": rent_burden if rent_burden else {r: 30.0 + (i - 8) * 2 for i, r in enumerate(NZ_REGIONS)},
        },
        "ranking": {
            "value": 8.0,
            "trend": "down",
        ...
        "demographics_gap": {
            "value": 25.0,
            "trend": "up",
            "by_region_growth": {r: 2.0 + (i - 8) * 0.3 for i, r in enumerate(NZ_REGIONS)},
    ...
    chart_data = {
        "ranking_bar": [
            {"region": r, "years": years_to_buy.get(r, nat_avg + (i - 8) * 1.5), "rank": i + 1}
            for i, r in enumerate(sorted(NZ_REGIONS, key=lambda x: years_to_buy.get(x, 8), reverse=True))
        ],
        "rent_burden_bar": [
        ...
        "scatter": [
            {"income": 60000 + i * 5000, "price": 500000 + i * 50000, "label": NZ_REGIONS[i], "region": NZ_REGIONS[i]}
        ...
        "migration_bar": [
            {"region": r, "value": 5000 + i * 500}
            for i, r in enumerate(NZ_REGIONS[:8])
        ],
        "heatmap_z": [[(nat_avg + i * 0.5 + j * 0.3) % 15 for j in range(12)] for i in range(len(NZ_REGIONS))],
