...
def create_macro_dashboard():
    ...
    return dbc.Container(
        ...
        children=[
            ...
            dcc.Store(
                id="macro-filter-state", data={"timeframe": "12M", "region": "All"}
            ),
        ],
    )
    ...
    return html.Div(
        [
            dbc.Row(
                [
                    dbc.Col(
                        MacroKPICard.create_ocr_card(**kpis["ocr"]), width=12, lg=4
                    ),
                    dbc.Col(
                        MacroKPICard.create_mortgage_rates_card(
                            **kpis["mortgage_rates"]
                        ),
                        width=12,
                        lg=4,
                    ...
                    dbc.Col(
                        MacroKPICard.create_mortgage_cost_card(**kpis["mortgage_cost"]),
                        width=12,
                        lg=4,
                    ),
                ],
            ...
            dbc.Row(
                [
                    dbc.Col(
                        MacroKPICard.create_construction_card(**kpis["construction"]),
                        width=12,
                        lg=6,
                    ),
                    dbc.Col(
                        MacroKPICard.create_ocr_listings_corr_card(
                            **kpis["ocr_listings_corr"]
                        ),
                        width=12,
                        lg=6,
                    ),
...
def _build_ocr_timeline(chart_data: dict) -> html.Div:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers+text",
            marker=dict(
                size=14, color=ACCENT["ocr"], line=dict(color="white", width=2)
            ),
    ...
    fig.update_layout(
        ...
        height=220,
        xaxis=dict(
            showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10), dtick=6
        ),
        yaxis=dict(
            title="OCR %",
            showgrid=True,
                    ...
                    dbc.Col(
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    html.H6(
                                        "Mortgage Yield Curve (Current vs 3M Ago)",
                                        style={
                                            "fontSize": "0.75rem",
                                            "fontWeight": "700",
                                            "textTransform": "uppercase",
                    ...
                    dbc.Col(
                        dbc.Card(
                            dbc.CardBody(
                                [
                                    html.H6(
                                        "Monthly Mortgage Cost by Suburb ($750K, 2Y)",
                                        style={
                                            "fontSize": "0.75rem",
                                            "fontWeight": "700",
                                            "textTransform": "uppercase",
...
def _make_suburb_cost_chart(suburbs: list, costs: list) -> go.Figure:
    ...
    fig = go.Figure(
        go.Bar(
            ...
            orientation="h",
            marker=dict(
                color=list(c), colorscale=[[0, "#dee2e6"], [1, ACCENT["cost"]]]
            ),
            text=[f"${v:,}" for v in c],
...
def _make_ocr_listings_scatter(scatter_data: list) -> go.Figure:
    ...
    for d in scatter_data:
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers+text",
                marker=dict(
                    size=12, color=ACCENT["ocr"], line=dict(color="white", width=1.5)
                ),
                text=[d["label"]],
                textposition="top center",
    ...
    z = np.polyfit(
        [d["ocr"] for d in scatter_data], [d["listings"] for d in scatter_data], 1
    )
    p = np.poly1d(z)
    x_range = np.linspace(
        min(d["ocr"] for d in scatter_data), max(d["ocr"] for d in scatter_data), 50
    )
    fig.add_trace(
        go.Scatter(
            x=x_range,
...
def _build_correlation_section(chart_data: dict) -> html.Div:
    matrix_fig = _make_correlation_matrix(
        chart_data["correlation_matrix"], chart_data["matrix_variables"]
    )
        ...
        [
            html.H5(
                ...
                style={
                    ...
                    "letterSpacing": "0.5px",
                    "color": "#12263A",
                    "borderLeft": "4px solid " + ACCENT["correlation"],
                    "paddingLeft": "10px",
                    "marginBottom": "10px",
                },
            ),
                                ...
                                [
                                    ...
                                    html.Div(
                                        [
                                            ...
                                            html.Span(
                                                "OCR cut → listings increase after 3-6 months",
                                                style={
                                                    "fontSize": "0.75rem",
                                                    "fontWeight": "600",
                                                    "color": ACCENT["correlation"],
                                                },
                                            ),
                                        ],
                                        className="mt-3",
...
def _make_lag_indicator(lag_data: dict) -> html.Div:
            ...
            step_elems.append(
                html.Div(
                    [
                        ...
                        html.Span(
                            "+3m", style={"fontSize": "0.6rem", "color": "#adb5bd"}
                        ),
                    ],
                    className="d-flex flex-column align-items-center",
                )
    ...
    return html.Div(
        step_elems, className="d-flex flex-row align-items-center flex-wrap gap-2"
    )
