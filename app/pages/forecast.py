...
def create_forecast_dashboard():
    ...
    return dbc.Container(
        ...
        children=[
            ...
            dcc.Store(id="forecast-filter-state", data={"timeframe": "12M", "scenario": "Base"}),
        ],
    )
...
def _build_hero_row(hero_kpis: dict) -> html.Div:
    ...
    return html.Div([
        dbc.Row([
            dbc.Col(ForecastKPICard.create_price_forecast_card(**kpis["price_forecast"]), width=12, lg=4),
            dbc.Col(ForecastKPICard.create_confidence_range_card(**kpis["confidence_range"]), width=12, lg=4),
            dbc.Col(ForecastKPICard.create_ocr_impact_card(**kpis["ocr_impact"]), width=12, lg=4),
        ], className="g-3 mb-3"),
        dbc.Row([
            dbc.Col(ForecastKPICard.create_tourism_impact_card(**kpis["tourism_impact"]), width=12, lg=4),
            dbc.Col(ForecastKPICard.create_risk_regions_card(**kpis["high_risk_regions"]), width=12, lg=4),
            dbc.Col(ForecastKPICard.create_model_confidence_card(**kpis["model_confidence"]), width=12, lg=4),
        ], className="g-3"),
    ], className="mb-4")
...
def _build_forecast_chart(chart_data: dict) -> html.Div:
    ...
    fig.add_trace(go.Scatter(
        x=list(range(len(fs["historical"]))), y=fs["historical"],
    ...
    fig.add_trace(go.Scatter(
        x=list(range(len(fs["historical"]), len(fs["historical"]) + len(fs["forecast"]))),
        y=fs["forecast"],
    ...
    fig.add_trace(go.Scatter(
        ...
        hoverinfo="skip", fillcolor="rgba(46,134,171,0.1)",
    ))
    ...
    fig.add_trace(go.Scatter(
        x=list(range(len(fs["historical"]), len(fs["historical"]) + len(fs["forecast"]))),
        y=fs["conf_80_high"],
        mode="lines", name="80% CI",
        line=dict(width=0), showlegend=False,
        hoverinfo="skip", fillcolor="rgba(46,134,171,0.2)",
    ))
    ...
    fig.add_vline(x=split_point - 0.5, line_dash="dot", line_color="#adb5bd", line_width=1.5)
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Median Price (NZD)", showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10), tickprefix="$"),
        hovermode="x unified",
    )

    return dbc.Card(dbc.CardBody([
        html.H6("12-Month Price Forecast with Confidence Bands", style={
        ...
        html.Div([
            html.Span("Blue = Historical  |  Dashed = Forecast  |  Shaded = Confidence Intervals",
                      style={"fontSize": "0.68rem", "color": "#8898aa"}),
        ], className="text-center mt-1"),
    ], className="p-3"), className="mb-4")
...
def _build_decomposition_section(chart_data: dict) -> html.Div:
    ...
    return html.Div([
        html.H5("Trend + Seasonality Decomposition", style={
            "fontSize": "0.85rem", "fontWeight": "800", "textTransform": "uppercase",
            "letterSpacing": "0.5px", "color": "#12263A", "borderLeft": "4px solid " + ACCENT["forecast"],
            "paddingLeft": "10px", "marginBottom": "12px",
        }),
        dbc.Card(dbc.CardBody([
            dcc.Graph(figure=trend_fig, config={"displayModeBar": False}, style={"height": "260px"}),
        ], className="p-3")),
    ], className="mb-4")
...
def _make_trend_chart(data: dict) -> go.Figure:
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", y=1.12, x=0.5, xanchor="center"),
        xaxis=dict(showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10)),
        yaxis=dict(title="Index", showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10)),
        hovermode="x unified",
    )
    return fig
...
def _build_scenario_comparison(chart_data: dict) -> html.Div:
    ...
    return html.Div([
        html.H5("Scenario Comparison Panel", style={
            "fontSize": "0.85rem", "fontWeight": "800", "textTransform": "uppercase",
            "letterSpacing": "0.5px", "color": "#12263A", "borderLeft": "4px solid " + ACCENT["ocr"],
            "paddingLeft": "10px", "marginBottom": "12px",
        }),
        dbc.Row([
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("OCR Scenario Impact on Price", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                    "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
                }),
            ...
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("DOM Forecast (12 Months)", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                    "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
                }),
                dcc.Graph(figure=dom_fig, config={"displayModeBar": False}, style={"height": "200px"}),
            ]), className="p-3"), className="col-12 col-lg-6"),
        ], className="g-3 mb-4"),
    ])
...
def _build_scenario_table(scenarios: dict) -> html.Div:
    ...
    for key in ["base", "optimistic", "stress"]:
        ...
        rows.append(html.Div([
            html.Span(label, style={"fontSize": "0.72rem", "fontWeight": "700", "width": "80px", "color": color}),
            html.Span(f"${s['price']:,.0f}", style={"fontSize": "0.78rem", "width": "90px", "textAlign": "right"}),
            html.Span(f"DOM {s['dom']}d", style={"fontSize": "0.72rem", "color": "#8898aa", "marginLeft": "8px"}),
            html.Span(f"L: {s['listings']:,}", style={"fontSize": "0.72rem", "color": "#8898aa", "marginLeft": "8px"}),
...
def _make_scenario_chart(scenarios: dict) -> go.Figure:
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Price (NZD)", showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=9)),
        showlegend=False,
    )
    return fig
...
def _build_risk_section(chart_data: dict) -> html.Div:
    heatmap_fig = _make_risk_heatmap(chart_data["regions"], chart_data["heatmap_z"], chart_data["heatmap_months"])

    return html.Div([
        html.H5("Risk Heatmap & Divergence Alerts", style={
            "fontSize": "0.85rem", "fontWeight": "800", "textTransform": "uppercase",
            "letterSpacing": "0.5px", "color": "#12263A", "borderLeft": "4px solid " + ACCENT["risk"],
            "paddingLeft": "10px", "marginBottom": "12px",
        }),
        dbc.Card(dbc.CardBody([
            html.H6("Forecast Risk by Region × Time (Blue=Stable, Orange=Moderate, Red=High)", style={
                "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
            }),
            dcc.Graph(figure=heatmap_fig, config={"displayModeBar": False}, style={"height": "280px"}),
        ], className="p-3")),
...
def _build_divergence_table(risk_data: list) -> dbc.Card:
    ...
    for item in risk_data:
        ...
        conf_color = "#28a745" if item["confidence"] >= 75 else "#ffc107" if item["confidence"] >= 65 else "#dc3545"
        rows.append(html.Tr([
            html.Td(item["region"], style={"fontWeight": "600", "fontSize": "0.82rem"}),
            html.Td(item["risk"], style={"color": risk_color, "fontWeight": "700", "fontSize": "0.82rem"}),
            html.Td(f"{item['confidence']}%", style={"color": conf_color, "fontWeight": "600", "fontSize": "0.82rem"}),
        ]))

    return dbc.Card(dbc.CardBody([
        html.H6("Divergence Alert Panel", style={
            "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
            "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
        }),
        html.Table([
            html.Thead(html.Tr([
                ...
                html.Th("Risk Level", style={"fontSize": "0.72rem"}),
                html.Th("Confidence", style={"fontSize": "0.72rem"}),
            ])),
            html.Tbody(rows),
