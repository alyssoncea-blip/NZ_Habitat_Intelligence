...
def create_tourism_dashboard():
    ...
    return dbc.Container(
        ...
        children=[
            ...
            # KPI grid
            html.Div(id="tourism-kpi-grid", children=create_tourism_kpi_grid(processed_kpis)),
            
            # Impact analysis
            html.Div(id="impact-analysis", children=create_impact_analysis_section()),
            
            # Insights
            html.Div(id="tourism-insights", children=create_tourism_insights(processed_kpis)),
            
            # Callbacks
            dcc.Store(id='tourism-kpis-data', data=processed_kpis.get("kpis", []) if isinstance(processed_kpis, dict) else processed_kpis.to_dict('records')),
            dcc.Store(id='tourism-filter-state', data={"timeframe": "12M", "region": "All"}),
        ]
    )
    ...
    for _, kpi in kpis_df.iterrows():
        ...
        if len(stats_data) < 4:
            stats_data.append({
                ...
                'value': round(float(kpi_value), 1) if isinstance(kpi_value, (int, float)) else kpi_value,
                'unit': kpi_unit,
...
def create_tourism_kpi_grid(kpis_df):
    ...
    for category, kpis in categorized.items():
        ...
        rows.append(
            dbc.Row(category_cards, className="kpi-grid-row")
        )
...
def create_impact_analysis_section():
    ...
    return dbc.Card([
        dbc.CardHeader([
            html.H5("Impact Analysis", className="analysis-title")
        ]),
        dbc.CardBody([
            html.P("Tourism impact analysis on housing market indicators.", 
                   className="text-muted"),
            html.Small("Future enhancement: Detailed correlation analysis between tourism metrics and housing market performance.",
...
def create_tourism_insights(kpis_df):
    ...
    return dbc.Card([
        dbc.CardHeader([
            html.H5("Tourism Insights", className="insights-title")
        ]),
        dbc.CardBody([
            html.Ul([
                html.Li(insight) for insight in insights[:3]
            ], className="insights-list"),
            html.P("Seasonal analysis recommended for tourism-dependent regions.",
...
def categorize_tourism_kpis(kpis_df):
    ...
    for _, kpi in kpis_df.iterrows():
        ...
        elif any(term in kpi_name for term in ['penetration', 'share', 'rental', 'airbnb']):
