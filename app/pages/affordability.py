...
def create_affordability_dashboard():
    ...
    return dbc.Container(
        ...
        children=[
            ...
            dcc.Store(id="affordability-filter-state", data={"timeframe": "12M", "region": "All"}),
        ],
    )
...
def _build_hero_row(hero_kpis: dict) -> html.Div:
    ...
    return html.Div([
        dbc.Row([
            dbc.Col(AffordabilityKPICard.create_years_to_buy_card(**kpis["years_to_buy"]), width=12, lg=4),
            dbc.Col(AffordabilityKPICard.create_rent_burden_card(**kpis["rent_burden"]), width=12, lg=4),
            dbc.Col(AffordabilityKPICard.create_ranking_card(**kpis["ranking"]), width=12, lg=4),
        ], className="g-3 mb-3"),
        dbc.Row([
            dbc.Col(AffordabilityKPICard.create_gap_card(**kpis["demographics_gap"]), width=12, lg=6),
            dbc.Col(AffordabilityKPICard.create_migration_card(**kpis["net_migration"]), width=12, lg=6),
        ], className="g-3"),
    ], className="mb-4")
...
def _build_ranking_section(chart_data: dict) -> html.Div:
    ...
    return html.Div([
        html.H5("Affordability Comparison", style={
            "fontSize": "0.85rem", "fontWeight": "800", "textTransform": "uppercase",
            "letterSpacing": "0.5px", "color": "#12263A", "borderLeft": "4px solid " + ACCENT["years"],
            "paddingLeft": "10px", "marginBottom": "12px",
        }),
        dbc.Row([
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("Affordability Ranking by Region (1=Most Affordable)", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                    "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
                }),
                dcc.Graph(figure=ranking_fig, config={"displayModeBar": False}, style={"height": "320px"}),
            ]), className="p-3"), className="col-12 col-lg-7"),
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("Rent Burden Distribution Across Regions", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                    "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
                }),
                dcc.Graph(figure=rent_burden_fig, config={"displayModeBar": False}, style={"height": "320px"}),
...
def _make_ranking_chart(data: list) -> go.Figure:
    colors = ["#28a745" if r["rank"] <= 4 else "#ffc107" if r["rank"] <= 10 else "#e74c3c"
              for r in data]
    ...
    fig.update_layout(
        ...
        height=320,
        xaxis=dict(title="Rank (1=Best)", dtick=1, showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10)),
...
def _make_rent_burden_chart(data: list) -> go.Figure:
    colors = ["#dc3545" if d["burden"] > 35 else "#ffc107" if d["burden"] > 28 else "#28a745"
              for d in data]
    fig = go.Figure(go.Bar(
    ...
    fig.update_layout(
        ...
        height=320,
        xaxis=dict(title="Rent Burden (%)", showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10)),
...
def _build_middle_charts(chart_data: dict) -> html.Div:
    ...
    return html.Div([
        html.H5("Drivers of Affordability", style={
            "fontSize": "0.85rem", "fontWeight": "800", "textTransform": "uppercase",
            "letterSpacing": "0.5px", "color": "#12263A", "borderLeft": "4px solid " + ACCENT["gap"],
            "paddingLeft": "10px", "marginBottom": "12px",
        }),
        dbc.Row([
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("Income vs House Price by Region", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                    "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
                }),
            ...
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("Population vs Housing Supply Growth", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                    "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
                }),
                dcc.Graph(figure=demo_fig, config={"displayModeBar": False}, style={"height": "260px"}),
            ]), className="p-3"), className="col-12 col-lg-4"),
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("Net Internal Migration by Region", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                    "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
                }),
                dcc.Graph(figure=migration_fig, config={"displayModeBar": False}, style={"height": "260px"}),
            ]), className="p-3"), className="col-12 col-lg-4"),
        ], className="g-3 mb-4"),
    ])
...
def _make_income_price_scatter(data: list) -> go.Figure:
    ...
    for d in data:
        fig.add_trace(go.Scatter(
            x=[d["income"]], y=[d["price"]],
            mode="markers+text",
            marker=dict(size=14, color=ACCENT["years"], line=dict(color="white", width=1.5)),
            text=[d["region"]],
    ...
    fig.add_trace(go.Scatter(
        x=[0, max_income], y=[0, max_price],
        mode="lines", line=dict(color="#dee2e6", width=2, dash="dash"),
        showlegend=False,
    ))
    fig.update_layout(
        plot_bgcolor="white", paper_bgcolor="white",
        margin=dict(l=50, r=20, t=20, b=50),
        xaxis=dict(title="Median Income (NZD)", showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10)),
        yaxis=dict(title="House Price (NZD)", showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10)),
    )
    return fig
...
def _make_demo_ts(data: dict) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=data["months"], y=data["population"],
        mode="lines", name="Population Growth",
    ...
    fig.add_trace(go.Scatter(
        x=data["months"], y=data["housing_supply"],
        mode="lines", name="Housing Supply",
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="% Growth", showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10)),
        hovermode="x unified",
    )
    return fig
...
def _make_migration_chart(data: list) -> go.Figure:
    colors = [ACCENT["migration"] if v > 0 else "#dc3545" for v in [d["value"] for d in data]]
    fig = go.Figure(go.Bar(
        x=[d["value"] for d in data],
        y=[d["region"] for d in data],
        orientation="h",
        marker=dict(color=colors),
        text=[f"+{d['value']:,}" if d["value"] > 0 else f"{d['value']:,}" for d in data],
        textposition="outside",
    ...
    fig.update_layout(
        ...
        height=260,
        xaxis=dict(title="Net Migration (people/yr)", showgrid=True, gridcolor="rgba(0,0,0,0.05)", tickfont=dict(size=10)),
        yaxis=dict(title="", tickfont=dict(size=10), gridcolor="rgba(0,0,0,0.05)"),
        showlegend=False,
    )
    return fig
...
def _build_heatmap_section(chart_data: dict) -> html.Div:
    ...
    return html.Div([
        html.H5("Affordability Pressure Heatmap", style={
            "fontSize": "0.85rem", "fontWeight": "800", "textTransform": "uppercase",
            "letterSpacing": "0.5px", "color": "#12263A", "borderLeft": "4px solid " + ACCENT["rent_burden"],
            "paddingLeft": "10px", "marginBottom": "12px",
        }),
        dbc.Row([
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("Affordability Score by Region × Time", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
                    "letterSpacing": "0.5px", "color": "#495057", "marginBottom": "8px",
                }),
                dcc.Graph(figure=heatmap_fig, config={"displayModeBar": False}, style={"height": "320px"}),
            ]), className="p-3"), className="col-12 col-lg-8"),
            dbc.Col(dbc.Card(dbc.CardBody([
                html.H6("Affordability Risk Indicator", style={
                    "fontSize": "0.75rem", "fontWeight": "700", "textTransform": "uppercase",
...
def _make_affordability_heatmap(regions: list, z_matrix: list, time_periods: list) -> go.Figure:
...
def _make_risk_indicator(ranking_data: list) -> html.Div:
    ...
    def risk_group(items: list, color: str, label: str) -> html.Div:
        ...
        return html.Div([
            ...
            html.Div(regions_list, style={"fontSize": "0.72rem", "color": "#495057", "lineHeight": "1.4"}),
        ], className="mb-3")
