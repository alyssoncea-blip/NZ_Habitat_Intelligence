...
def create_housing_dashboard():
    ...
    return dbc.Container([
        ...
        html.Div([
            dbc.Row([
                ...
                dbc.Col(HousingKPICard.create_supply_gap_card(
        ...
        html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    html.Div([
                        html.H6("Price Distribution by Suburb",
                                className="mb-2", style={"fontSize": "0.9rem", "fontWeight": "600"}),
                        dcc.Graph(
                            id="housing-boxplot",
                            figure=_create_boxplot_figure(chart_data),
                            config={"displayModeBar": False},
                            style={"height": "280px"}
                        )
                    ], className="chart-card p-3")
                ], width=12, md=7, className="mb-3"),

                dbc.Col([
                    html.Div([
                        html.H6("Listings Volume Trend",
                                className="mb-2", style={"fontSize": "0.9rem", "fontWeight": "600"}),
                        dcc.Graph(
                            id="housing-line",
                            figure=_create_line_figure(chart_data["line_chart"]),
                            config={"displayModeBar": False},
                            style={"height": "280px"}
                        )
                    ], className="chart-card p-3")
            ...
            dbc.Row([
                dbc.Col([
                    html.Div([
                        html.H6("Property Type Breakdown",
                                className="mb-2", style={"fontSize": "0.9rem", "fontWeight": "600"}),
                        dcc.Graph(
                            id="housing-stacked-bar",
                            figure=_create_stacked_bar_figure(chart_data["stacked_bar"]),
                            config={"displayModeBar": False},
                            style={"height": "240px"}
                        )
                    ], className="chart-card p-3")
                ...
                dbc.Col([
                    html.Div([
                        html.H6("Price per m² vs Bedrooms",
                                className="mb-2", style={"fontSize": "0.9rem", "fontWeight": "600"}),
                        dcc.Graph(
                            id="housing-scatter",
                            figure=_create_scatter_figure(chart_data["scatter"]),
                            config={"displayModeBar": False},
                            style={"height": "240px"}
                        )
                    ], className="chart-card p-3")
                ...
                dbc.Col([
                    html.Div([
                        html.H6("Market Speed (DOM Distribution)",
                                className="mb-2", style={"fontSize": "0.9rem", "fontWeight": "600"}),
                        dcc.Graph(
                            id="housing-dom-histogram",
                            figure=_create_histogram_figure(chart_data["dom_histogram"]),
                            config={"displayModeBar": False},
                            style={"height": "240px"}
                        )
                    ], className="chart-card p-3")
        ...
        html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    html.Div([
                        html.H6("Supply vs Demand Heatmap",
                                className="mb-2", style={"fontSize": "0.9rem", "fontWeight": "600"}),
                        dcc.Graph(
                            id="housing-heatmap",
                            figure=_create_heatmap_figure(chart_data),
                            config={"displayModeBar": False},
                            style={"height": "280px"}
                        )
                    ], className="chart-card p-3")
                ...
                dbc.Col([
                    html.Div([
                        html.H6("Consents vs Listings",
                                className="mb-2", style={"fontSize": "0.9rem", "fontWeight": "600"}),
                        dcc.Graph(
                            id="housing-consents-listings",
                            figure=_create_consents_figure(data["supply_demand"]),
                            config={"displayModeBar": False},
                            style={"height": "280px"}
                        )
                    ], className="chart-card p-3")
    ...
    ], fluid=True, className="housing-dashboard py-4",
       style={"backgroundColor": "#f8f9fa", "minHeight": "100vh", "paddingLeft": "24px", "paddingRight": "24px"})
        ...
        dbc.Col([
            html.Div([
                ...
                dcc.Dropdown(
                    ...
                    options=[{"label": c, "value": c} for c in ["All"] + CITIES_LIST],
                    value="All",
                ...
                dcc.Dropdown(
                    ...
                    options=[{"label": s, "value": s} for s in ["All"] + SUBURBS_LIST],
                    value="All",
                ...
                dbc.ButtonGroup([
                    dbc.Button("3M", id="housing-btn-3m", color="outline-primary", size="sm", active=True),
                    dbc.Button("6M", id="housing-btn-6m", color="outline-primary", size="sm"),
                    dbc.Button("12M", id="housing-btn-12m", color="outline-primary", size="sm"),
                ], size="sm"),
...
def _create_boxplot_figure(chart_data):
    ...
    colors = ["#2E86AB", "#A23B72", "#F18F01", "#28a745", "#6c757d",
              "#17a2b8", "#bc5090", "#ff6f00", "#58595b", "#003f5c"]
    ...
    fig.update_layout(
        ...
        showlegend=False,
        font=dict(size=10),
        xaxis=dict(title="Median Price (NZD)", gridcolor="#f0f0f0", tickprefix="$", tickformat=",.0f"),
...
def _create_stacked_bar_figure(stacked_bar_data):
    ...
    for col, color in zip(["House", "Apartment", "Townhouse"],
...
def _create_scatter_figure(scatter_data):
    ...
    colors_map = {
        "Ponsonby": "#2E86AB", "Mt Eden": "#A23B72", "Newmarket": "#F18F01",
        "Kelburn": "#28a745", "Miramar": "#6c757d", "Riccarton": "#17a2b8",
        "New Brighton": "#bc5090", "Hillcrest": "#ff6f00", "North East Valley": "#58595b",
    ...
    for suburb in set(d["suburb"] for d in scatter_data):
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers',
            name=suburb,
            marker=dict(size=10, color=colors_map.get(suburb, "#6c757d"), line=dict(width=1, color='white')),
...
def _create_consents_figure(supply_demand):
    ...
    fig.add_trace(go.Bar(
        x=labels, y=consents, name="Building Consents",
        marker_color="#2E86AB", opacity=0.7,
        hovertemplate='%{y:,} consents<extra></extra>',
    ), secondary_y=False)

    fig.add_trace(go.Scatter(
        x=labels, y=listings, name="Listings Volume",
