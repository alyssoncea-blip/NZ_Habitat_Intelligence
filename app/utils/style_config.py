...
def get_kpi_color(kpi_name, value, category=None):
    ...
    elif any(term in kpi_lower for term in ['growth', 'increase', 'rate', 'yield', 'return']):
        if val_float > 5:
    ...
    elif any(term in kpi_lower for term in ['deficit', 'gap', 'risk', 'volatility', 'uncertainty']):
        if val_float < 10:
    ...
    elif '%' in str(value) or any(term in kpi_lower for term in ['ratio', 'penetration', 'share']):
        if val_float < 10:
            return "Low", COLORS['success']['primary']
        elif val_float < 30:
            return "Moderate", "#ffc107"
    ...
    elif any(term in kpi_lower for term in ['price', 'cost', 'value', 'housing', 'rent']):
    ...
    # 6. Fallback baseado em category
        ...
        if category in ['economic', 'growth']:
            return "Positive" if val_float > 0 else "Challenging", COLORS['success']['primary'] if val_float > 0 else COLORS['danger']['primary']
        elif category in ['risk', 'volatility']:
            color = COLORS['danger']['primary'] if val_float > 50 else COLORS['warning']['primary'] if val_float > 25 else COLORS['success']['primary']
            status = "High Risk" if val_float > 50 else "Moderate Risk" if val_float > 25 else "Low Risk"
            return status, color
