...
class AffordabilityKPICalculator(KPIBaseCalculator):
    ...
    def calc(self) -> pd.DataFrame:
        ...
        if years_data:
            ...
            rows.append(
                {
                    "name": "Years to Buy (National Avg)",
                    "value": nat_years,
                    "unit": "years",
        ...
        for rank, d in enumerate(ranked, 1):
            rows.append(
                {
                    "name": f"Affordability Rank - {d['region']}",
                    "value": rank,
                    "unit": "/16",
                    "description": f"Affordability ranking for {d['region']}",
                    "category": "Affordability",
                    "source": "Years-to-buy calculation",
                }
            )
        ...
        if df is not None and not df.empty and "population_growth_yoy" in df.columns:
            ...
            if not valid.empty:
                ...
                rows.append(
                    {
                        "name": "Net Migration (National)",
                        "value": float(net_migration),
                        "unit": "people/yr",
                        "description": "Estimated net migration",
                        "category": "Affordability",
                        "source": "Stats NZ population growth",
                    }
                )
