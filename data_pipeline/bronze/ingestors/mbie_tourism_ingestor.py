...
class MBIEIngestor:
    ...
    def fetch_visitor_arrivals(self) -> Dict[str, Any]:
        ...
        df = self._fetch_with_fallback(
            MBIE_ENDPOINTS["visitor_arrivals"], "visitor arrivals"
        )
        if df is not None and not df.empty:
            return {
                "metadata": {
                    "source": "MBIE/Stats NZ (Live)",
        ...
        logger.info(
            "  Visitor arrivals: using real historical data (%d records)",
            len(_VISITOR_ARRIVALS),
        )
        return {
            "metadata": {
                "source": "MBIE/Stats NZ (Historical)",
                "date_fetched": datetime.now().isoformat(),
    ...
    def fetch_regional_tourism(self) -> Dict[str, Any]:
        ...
        df = self._fetch_with_fallback(
            MBIE_ENDPOINTS["regional_tourism"], "regional tourism"
        )
        if df is not None and not df.empty:
            return {
                "metadata": {
        ...
        # Fallback: real historical data
        logger.info(
            "  Regional tourism: using real historical data (%d records)",
            len(_REGIONAL_TOURISM),
        )
        return {
            "metadata": {
                "source": "MBIE (Historical)",
                "date_fetched": datetime.now().isoformat(),
    ...
    def fetch_rent_data(self) -> Dict[str, Any]:
        ...
        # Fallback: real historical data
        logger.info(
            "  Rent data: using real historical data (%d records)", len(_REAL_RENT_DATA)
        )
        return {
            "metadata": {
                "source": "Tenancy Services/MBIE (Historical)",
                "date_fetched": datetime.now().isoformat(),
