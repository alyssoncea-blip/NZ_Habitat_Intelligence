...
sources = [
    ...
    ('Stats NZ ArcGIS root', 'https://services2.arcgis.com/Iu3D1lHt5NUkVU3S/arcgis/rest/services/Regional_Council_2024/FeatureServer', None),
    
    # 2. Try different Stats NZ service names
    ('Stats NZ RC2024', 'https://services2.arcgis.com/Iu3D1lHt5NUkVU3S/arcgis/rest/services/Regional_Council_2024/FeatureServer/0', None),
    
    # 3. Try data.govt.nz API
    ('data.govt.nz API', 'https://data.govt.nz/api/3/action/package_search', {'q': 'regional council boundaries'}),
    
    # 4. Try Stats NZ data portal
    ('Stats NZ portal', 'https://www.stats.govt.nz/tools-and-services/datafinder/', None),
    
    # 5. Try LINZ Data Service
    ('LINZ WFS', 'https://data.linz.govt.nz/services;key=/wfs', {
        'service': 'WFS',
                ...
                try:
                    ...
                    if 'features' in data:
                        ...
                        if n > 0:
                            ...
                            with open('app/assets/nz_regions_official.geojson', 'w') as f:
                                json.dump(data, f)
                            print('  ** SAVED **')
                            break
                        ...
                        for item in data['results'][:3]:
                            ...
                            for res in resources:
                                if res.get('format', '').lower() == 'geojson':
                                    print(f'      GeoJSON resource: {res.get("url", "?")}')
                                    # Try to download it
                                    r2 = requests.get(res['url'], timeout=15, headers={'User-Agent': 'Mozilla/5.0'})
